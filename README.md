...
- LSP handlers (hover, completion, diagnostics, etc.) are located in `src/lsp/`, while diagnostic processing is split into modular handler modules (`src/asm/diagnostics/labels.ts`, `expressions.ts`, `general.ts`, `types.ts`) re-exported through `src/asm/diagnostics.ts` for isolated testing and clear maintenance.
...
- The `test/fixtures/invalid/` directory (including `macro-recursion-limit.S` and `macro-deep-nested-calls.S`) and `examples/DIAGNOSTICS.S` contain negative samples covering 17 of 21 diagnostic codes (including forward macro/equate calls, macro arity mismatch, recursion limits, deep expansion limits, unclosed macros, unterminated loops, and invalid operands); the showcase and fixture tests assert the exact expected diagnostic-code sequence for every invalid fixture.
...
- Macro argument substitution lives in `expandMacroCall` in `src/asm/expansion.ts`, mapping call arguments onto the `]n` placeholders they satisfy inside parsed macro bodies; argument splitting, arity checking, and signature-help parameter tracking all share the one `forEachMacroArgumentToken` separator walker exported from the same module, while `CALL_SITE_TOKEN_KEY` and `getCallSiteToken` in `src/asm/expansion.ts` (re-exported by `src/lsp/utils.ts`) provide unified call-site token property key definition and retrieval across diagnostic and navigation modules.
...
- Local-label resolution is limited to ordinary Merlin anchor-based locals; local labels inside macros are rejected to match Merlin32 behavior. Centralized resolution helpers in `src/asm/local-labels.ts` (`qualifyLocalName`, `getLocalLabelReference`, `getLocalLabelDefinition`, `getLocalLabelTargetLine`, `isLocalLabelResolved`, `getLocalLabelToken`) unify lookups and eliminate duplicated qualification string formatting across diagnostics, symbol navigation, and semantic tokens.
...
- Workspace `USE` resolution accepts `initializationOptions.merlinMacroFolder`, strips the legacy leading numeric subfolder, and tries an implicit `.s` suffix while preserving open-buffer and disk-cache precedence; `readMacroFolder` validates options via `isKnownInitializationOptionKey` and emits warnings for unknown keys.
...
- Guardrails catch and diagnose unresolvable or ambiguous cases: recursion (`macro-recursion`), deep nesting and branching expansion (`deep-macro-expansion`), token-pasted/generated names (`token-pasted-name`), and conditionals (`unresolved-conditional`), failing predictably without returning incorrect results. `MAX_MACRO_EXPANSION_DEPTH`, `MAX_MACRO_EXPANSION_LINES`, and `MAX_EXPRESSION_DEPTH` in `src/asm/limits.ts` keep the diagnostic, expansion, and expression parsing guards aligned: each call site stops expanding after the total line budget and reports one `deep-macro-expansion` diagnostic, while expression parsing prevents stack overflow crashes on deeply nested expressions.
