...
`examples/EXAMPLE.S` stays a diagnostics-free feature showcase (clean starter file) and `examples/DIAGNOSTICS.S` holds intentionally invalid syntax covering every diagnostic category; `test/examples-showcase.test.ts` asserts the split: the clean file must produce zero diagnostics and the showcase file must emit all eight diagnostic codes.
