...
The current diagnostics pass now also emits macro-specific failures for unsupported-instruction-or-undefined-macro call sites, forward macro calls and `EQU` references, duplicate macro definitions, missing `eom`/`<<<` terminators, illegal nested macro definitions, and obvious arity mismatches; ordinary labels retain forward-reference support.
