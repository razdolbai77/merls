...
The current expression parser recognizes decimal, hexadecimal, and separator-formatted binary literals, current-address `*`, left-to-right Merlin arithmetic, brace-controlled algebraic precedence, comparison/logical operators, modifiers, and indexed operands; string payloads strip the closing quote only when present so unterminated strings keep their full text. `test/expression.test.ts` covers each form.
