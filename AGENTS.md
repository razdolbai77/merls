...
The current workspace indexer now lives under `src/asm/workspace.ts`, follows `asm`/`put`/`use` directives across the local fixture corpus, and skips dependency resolution for `untitled:` buffers because virtual document URIs have no filesystem directory.
